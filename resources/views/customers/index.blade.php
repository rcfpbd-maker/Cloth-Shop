    ...
    <x-slot name="header">
        <div class="flex justify-between items-center text-slate-800">
            ...
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open" @click.away="open = false" class="btn-white flex items-center text-xs">
                    <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                    Export Contacts
                </button>
                <div x-show="open" x-transition class="absolute right-0 mt-2 w-36 bg-white rounded-xl shadow-xl shadow-slate-200/50 border border-slate-100 z-50 overflow-hidden">
                    <a href="{{ url('api/export/customers?format=excel') }}" target="_blank" class="block px-4 py-3 text-sm font-bold text-slate-700 hover:bg-emerald-50 hover:text-emerald-700 border-b border-slate-100 flex items-center">
                        Excel (CSV)
                    </a>
                </div>
            </div>
